      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-10 md:gap-12 items-start mb-12 md:mb-16">
          ...
          >
            ...
            <p className="text-lg text-silver-300 mb-4 leading-relaxed font-light">
              Co-founder di <a href="https://wesync.dev/" target="_blank" rel="noopener noreferrer" className="text-silver-200 hover:text-silver-100 underline">Wesync</a>, azienda specializzata in soluzioni cloud e sviluppo software innovativo.
            </p>
            <p className="text-lg text-silver-300 mb-8 leading-relaxed font-light">
          ...
          >
            {[
              ...
              <motion.div
                key={stat.label}
                className="glass-effect p-6 rounded-xl text-center border border-white/10 hover:border-silver-400/30 transition-all duration-300"
                initial={{ opacity: 0, scale: 0.8 }}
                animate={isInView ? { opacity: 1, scale: 1 } : {}}
                transition={{ duration: 0.6, delay: 0.6 + index * 0.1, ease: [0.22, 1, 0.36, 1] }}
                whileHover={{ scale: 1.05, y: -5 }}
              >
                <div className="text-4xl font-bold text-gradient-silver mb-2">
                  {stat.value}
                </div>
                <div className="text-sm text-silver-400 font-light">{stat.label}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {skills.map((skill, index) => (
              <motion.a
                key={skill.title}
                href={skill.href}
                className="group glass-effect p-6 rounded-xl border border-white/10 hover:border-silver-400/30 transition-all duration-300"
                initial={{ opacity: 0, y: 30 }}
                animate={isInView ? { opacity: 1, y: 0 } : {}}
                transition={{ duration: 0.6, delay: 0.8 + index * 0.1 }}
                whileHover={{ y: -5, scale: 1.02 }}
              >
                <h4 className="text-lg font-semibold text-gray-100 mb-3 group-hover:text-silver-200 transition-colors">
                  {skill.title}
                </h4>
                <p className="text-sm text-silver-400 mb-4 leading-relaxed font-light">
                  {skill.description}
                </p>
