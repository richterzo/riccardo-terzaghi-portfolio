        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-10 lg:gap-12 items-start mb-10 md:mb-12 lg:mb-16">
          ...
          >
            {[
              ...
              <motion.div
                key={stat.label}
                className="glass-effect p-4 sm:p-5 md:p-6 rounded-xl text-center border border-white/10 hover:border-silver-400/30 transition-all duration-300"
                initial={{ opacity: 0, scale: 0.8 }}
                animate={isInView ? { opacity: 1, scale: 1 } : {}}
                transition={{ duration: 0.6, delay: 0.6 + index * 0.1, ease: [0.22, 1, 0.36, 1] }}
                whileHover={{ scale: 1.05, y: -5 }}
              >
                <div className="text-3xl sm:text-4xl font-bold text-gradient-silver mb-1.5 md:mb-2">
                  {stat.value}
                </div>
                <div className="text-xs sm:text-sm text-silver-400 font-light">{stat.label}</div>
              </motion.div>
            ))}
          </motion.div>
        </div>
